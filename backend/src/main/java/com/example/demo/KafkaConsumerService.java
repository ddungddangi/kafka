...
public class KafkaConsumerService {
    ...
    public void listen(String message) {
        try {
            ...
            logger.info("[KafkaConsumerService] [4] [수신] Kafka Broker → Backend Consumer | payload='{}'", message);

            // 6. WebSocket 브로드캐스트
            ChatWebSocketHandler.broadcast(message);

            logger.info("[KafkaConsumerService] [5] [브로드캐스트] Backend Consumer → WebSocket Sessions | payload='{}'", message);
        } catch (Exception e) {
